...
def load_autism_model():
    ...
    try:
        ...
        if not os.path.exists(model_path):
            logger.warning(f"Model file not found: {model_path}. Running in test mode.")
            return False
...
def mock_prediction(processed_image):
    """Mock prediction for testing without TensorFlow"""
    # Generate a random-like prediction based on image properties
    img_sum = np.sum(processed_image)
    # Use image properties to generate a "prediction"
    prediction = (img_sum % 1000) / 1000.0  # Normalize to [0, 1]
    return prediction
...
def health_check():
    ...
    return jsonify({
        ...
        "tensorflow_available": model is not None,
...
def predict_autism():
    ...
    try:
        ...
        if model is not None:
            ...
            logger.info("Making prediction with TensorFlow model...")
            prediction = model.predict(processed_image)[0][0]
        else:
            # Mock prediction for testing
            logger.info("Making mock prediction (TensorFlow not available)...")
...
if __name__ == '__main__':
    ...
    if load_autism_model():
        logger.info("Backend ready with TensorFlow model!")
    else:
        logger.info("Backend ready in test mode (without TensorFlow). Install TensorFlow for real predictions.")
